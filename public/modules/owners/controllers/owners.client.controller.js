...
angular.module('owners').controller('OwnersController', ['$scope', '$stateParams', '$location', 'Authentication', 'Owners', '$modal', '$http',
	function($scope, $stateParams, $location, Authentication, Owners, $modal, $http ) {
        ...
        //change to native for loop
        $scope.getNumPosition=function(position, owner){
            var posAmt='broken';
            angular.forEach(owner.paidPlayer, function(playerArray){
                //console.log(playerArray.name);
                
                if(playerArray.name===position){
                    posAmt = playerArray.roster.length;
                }
            });
            return posAmt;
        };
...
//              controller: ReviewRosterController,
//              size: 'lg',
//              resolve: {
//                items: function () {
